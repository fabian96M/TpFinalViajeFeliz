...
class Viaje {
    ...
    public function carga($destino, $maxPasajeros, $idEmpresa, $numEmpleado, $importe) {
        ...
        /* Si la funcion se invoca en un viaje con un idViaje precargado, se listaran los pasajeros asociados al id y se guardara la coleccion */
        if ($this->getIdViaje() > 0) {
            $objPasajero = new Pasajero();
            $colPasajeros = array();
            /* se realiza la busqueda de pasajeros asociados en caso de que se trate de un listado */
            $colPasajeros = $objPasajero->listar(" idviaje = " . $this->getIdViaje());
            $this->setArrayPasajeros($colPasajeros);
        }
        ...
        if ($base->Iniciar()) {
            if ($base->Ejecutar($consultaViaje)) {
                if ($row2 = $base->Registro()) {
                    ...
                    $objResponsable = new Responsable;
                    $empresa = new Empresa;
                    /* se buscan los pasajeros asociados al viaje */
                    $arrayPas = $objPasajero->listar(" idviaje = " . $this->getIdViaje());
                    /* si hay pasajeros asociados */
                    if($arrayPas != null && count($arrayPas)>0){
                        $this->setArrayPasajeros($arrayPas);
    ...
    public function modificar() {
        ...
        $consultaModifica .= "vdestino = '" . $this->getDestino() . "', ";
        $consultaModifica .= "vcantmaxpasajeros = " . $this->getMaxPasajeros() . ", ";
        $consultaModifica .= "idempresa = " . $this->getObjEmpresa()->getId() . ", ";
        $consultaModifica .= "rnumeroempleado = " . $this->getObjResponsable()->getNumEmpleado() . ", ";
        $consultaModifica .= "vimporte = " . $this->getImporte() . " ";
        $consultaModifica .= "WHERE idviaje = " . $this->getIdViaje();
        if ($base->Iniciar()) {
    ...
    public function __toString() {
        ...
        $datosViaje .= "\n Cantidad Pasajeros: " . count($this->getArrayPasajeros()) . "\n";
        return $datosViaje;
    }
}
