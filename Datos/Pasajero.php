...
class Pasajero {
    ...
    public function carga($idViaje, $nombre, $apellido, $telefono, $documento) {
        /* si al pasajero se le inserta una idViaje para asociarlo a un viaje */
        if ($idViaje > 0) {
            $objViaje = new Viaje();
    ...
    public function modificar() {
        ...
        $consultaModifica .= "pnombre = '" . $this->getNombre() . "', ";
        $consultaModifica .= "papellido = '" . $this->getApellido() . "', ";
        $consultaModifica .= "ptelefono = " . $this->getTelefono() . " ";
        $consultaModifica .= "WHERE pdocumento = " . $this->getDocumento();
        if ($base->Iniciar()) {
    ...
    public function modificarPasajero($atributo, $nuevaInstancia) {
        switch ($atributo) {
                /* modificar nombre */
            case 1:
                $this->setNombre($nuevaInstancia);
                $confirmacion = true;
                break;
                /* modificar apellido */
            case 2:
                $this->setApellido($nuevaInstancia);
                $confirmacion = true;
                break;
                /* modificar documento */
            case 3:
                $this->setDocumento($nuevaInstancia);
                $confirmacion = true;
                break;
                /* Modificar numero de telefono */
            case 4:
                $this->setTelefono($nuevaInstancia);
                $confirmacion = true;
                break;
            default:
                $confirmacion = false;
